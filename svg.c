...
void printSvgObj(FILE* fp, svgObj* t, double dX, double dY, double scale){
	...
	if(t->type == 'R'){
		...
		fprintf(fp, "<rect x=\"%lf\" y=\"%lf\" width=\"%lf\" height=\"%lf\" %s stroke-width=\"0.5\"/>\n", (r->x1 + dX)*scale, (r->y1 + dY)*scale, (r->x2 - r->x1)*scale, (r->y2 - r->y1)*scale, paint);
	}else if(t->type == 'L'){
		struct svgLine* l = (&t->line);
		fprintf(fp, "<line x1=\"%lf\" y1=\"%lf\" x2=\"%lf\" y2=\"%lf\" stroke-width=\"0.5\" stroke=\"%s\"/>\n", (l->x1 + dX)*scale, (l->y1 + dY)*scale, (l->x2 + dX)*scale, (l->y2 + dY)*scale, svgColorMap[t->c]);
	}else if(t->type == 'C'){
		...
		fprintf(fp, "<circle cx=\"%lf\" cy=\"%lf\" r=\"%lf\" %s stroke-width=\"0.2\"/>\n", (c->x + dX)*scale, (c->y + dY)*scale, c->r*scale, paint);
	}else if(t->type == 'S'){
		...
		struct svgText* m = (&t->text);
		fprintf(fp, "<text x=\"%lf\" y=\"%lf\" fill=\"%s\" font-size=\"%lf\">%s</text>\n", (m->x + dX)*scale, (m->y + dY)*scale, svgColorMap[t->c], m->s * scale, m->msg);
	}
}
...
void addSvgObj(svg* t, svgObj* obj){
	...
	if(obj->type == 'R'){
		...
		x1 = obj->text.x;
		y1 = obj->text.y;
		x2 = x1+obj->text.s*strlen(obj->text.msg);
		y2 = y1+obj->text.s;
		if(x1 < t->minX){
			t->minX = x1;
		}
...
extern svgObj* newSvgText(char* msg, double x, double y, double s, svgColor c, int depth){
	svgObj* ret = malloc(sizeof(svgObj));
	strncpy(ret->text.msg, msg, 81);
	ret->text.msg[80] = 0;
	ret->type = 'T';
	ret->depth = depth;
	ret->text.x = x;
